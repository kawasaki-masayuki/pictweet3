...
RSpec.describe 'ツイート編集', type: :system do
  ...
  context 'ツイート編集ができるとき' do
    it 'ログインしたユーザーは自分が投稿したツイートの編集ができる' do
      ...
      # 投稿内容を編集する
      fill_in 'tweet_title', with: "#{@tweet1.text}+編集したタイトル"
      ...
      expect(page).to have_content("#{@tweet1.text}+編集したタイトル")
    end
  end
...
RSpec.describe 'ツイート詳細', type: :system do
  ...
  it 'ログインしたユーザーはツイート詳細ページに遷移してコメント投稿欄が表示される' do
    ...
    expect(page).to have_no_selector ".content_post[style='background-image: url(#{@tweet.image});']"
